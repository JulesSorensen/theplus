...
import { View, Text, TextInput, TouchableOpacity, FlatList, StyleSheet, Animated, SafeAreaView, Pressable } from 'react-native';
...
const HomeScreen = () => {
    ...
    const addGroup = () => {
        if (groupName.trim()) {
            ...
            navigation.navigate('GroupInvit', { groupName: newGroup.name, groupId: newGroup.id });
        }
    };
    ...
    const renderMessage = ({ item }) => {
        return (
            <View style={[
                ...
                <Text style={item.type === 'sent' ? styles.sentMessageText : styles.receivedMessageText}>{item.content}</Text>
    ...
    return (
        <SafeAreaView style={styles.safeContainer}>
            <View style={styles.container}>
                ...
                <Animated.View style={[styles.sideMenu, { left: menuAnimation, zIndex: 2 }]}>
                    <Text style={styles.menuTitle}>Groupes</Text>
                    <FlatList
                        data={groups}
                        keyExtractor={(item) => item.id}
                        renderItem={({ item }) => (
                            <TouchableOpacity onPress={() => openGroupInvit(item)} style={styles.groupItem}>
                    ...
                    {isAddGroupVisible ? (
                        ...
                        <TouchableOpacity style={styles.addGroupToggle} onPress={() => setAddGroupVisible(true)}>
